...
vi.mock('@gitroom/helpers/utils/custom.fetch', () => ({ useFetch: () => mocks.fetcher }));
vi.mock('@gitroom/react/translation/get.transation.service.client', () => ({
  useT: () => (_key: string, fallback: string) => fallback,
}));
vi.mock('@gitroom/react/toaster/toaster', () => ({ useToaster: () => ({ show: mocks.show }) }));
vi.mock('@gitroom/react/form/input', () => ({
  Input: ({ value, onChange, placeholder, name }: React.InputHTMLAttributes<HTMLInputElement>) => (
    <input aria-label={name} value={value} onChange={onChange} placeholder={placeholder} />
  ),
}));
...
const fillAndRegister = () => {
  fireEvent.change(screen.getByLabelText('agentName'), { target: { value: 'MyAgent' } });
...
describe('MoltbookProvider claim guide', () => {
  ...
  it('opens a validated claim URL and posts the secret in the status body', async () => {
    mocks.fetcher
      .mockResolvedValueOnce(response({ apiKey: 'secret-key', claimUrl: 'https://www.moltbook.com/claim/token' }))
      .mockResolvedValueOnce(response({ claimed: true }));
    const onComplete = vi.fn();
    render(<MoltbookProvider onComplete={onComplete} nonce="nonce_123" />);
    fillAndRegister();
    expect(await screen.findByRole('link', { name: /open claim page/i })).not.toBeNull();
    await waitFor(() => expect(onComplete).toHaveBeenCalledWith('secret-key', 'nonce_123'));
    expect(mocks.fetcher).toHaveBeenNthCalledWith(2, '/integrations/moltbook/status', {
      method: 'POST',
  ...
  it('rejects an untrusted claim URL without exposing upstream details', async () => {
    mocks.fetcher.mockResolvedValueOnce(response({ apiKey: 'secret-key', claimUrl: 'https://evil.test/claim/token' }));
    render(<MoltbookProvider onComplete={vi.fn()} nonce="nonce_123" />);
    fillAndRegister();
    expect(await screen.findByText('Moltbook returned an invalid claim link. Try again.')).not.toBeNull();
    expect(mocks.fetcher).toHaveBeenCalledTimes(1);
  });

  it('uses a safe registration error', async () => {
    mocks.fetcher.mockResolvedValueOnce(response({ error: 'upstream secret details' }, false));
    render(<MoltbookProvider onComplete={vi.fn()} nonce="nonce_123" />);
    fillAndRegister();
    expect(await screen.findByText('Could not create the Moltbook agent. Try again.')).not.toBeNull();
  ...
  it('times out after two minutes and can check again', async () => {
    ...
    expect(screen.getByText('We have not received the claim confirmation yet.')).not.toBeNull();
    fireEvent.click(screen.getByRole('button', { name: /check again/i }));
    expect(mocks.fetcher).toHaveBeenLastCalledWith('/integrations/moltbook/status', expect.any(Object));
  });
  ...
  it('ignores claim success after unmount', async () => {
    ...
    const onComplete = vi.fn();
    const view = render(<MoltbookProvider onComplete={onComplete} nonce="nonce_123" />);
    fillAndRegister();

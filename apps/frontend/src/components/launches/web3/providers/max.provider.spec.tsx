...
describe('MaxProvider connection guide', () => {
  ...
  it('shows the bot, permissions, and command before polling', () => {
    ...
    expect(
      screen
        .getByRole('link', { name: /open the vezdepost bot/i })
        .getAttribute('href')
    ).toBe('https://max.ru/vezdepost_bot');
    expect(
      screen.getByText(/read all messages and write messages/i)
    ).not.toBeNull();
  ...
  it('completes a verified connection', async () => {
    mocks.fetcher.mockResolvedValueOnce(
      response({ status: 'ready', chatId: 321 })
    );
  ...
  it('rechecks a discovered chat after fixing administrator status', async () => {
    mocks.fetcher
      .mockResolvedValueOnce(
        response({ status: 'bot_not_admin', candidateChatId: 321 })
      )
    ...
    fireEvent.click(screen.getByRole('button', { name: /check connection/i }));
    expect(
      await screen.findByText('The bot is not an administrator yet.')
    ).not.toBeNull();
    fireEvent.click(screen.getByRole('button', { name: /check again/i }));
    await waitFor(() => {
      expect(mocks.fetcher).toHaveBeenLastCalledWith(
        expect.stringContaining('chatId=321')
      );
      expect(onComplete).toHaveBeenCalledWith('321', 'nonce_123');
    });
  });
  ...
  it('explains missing MAX permissions', async () => {
    mocks.fetcher.mockResolvedValueOnce(
      response({ status: 'missing_permissions', candidateChatId: 321 })
    );
    render(<MaxProvider onComplete={vi.fn()} nonce="nonce_123" />);
    fireEvent.click(screen.getByRole('button', { name: /channel/i }));
    fireEvent.click(screen.getByRole('button', { name: /check connection/i }));
  ...
  it('ignores a response after unmount', async () => {
    ...
    const onComplete = vi.fn();
    const view = render(
      <MaxProvider onComplete={onComplete} nonce="nonce_123" />
    );
    fireEvent.click(screen.getByRole('button', { name: /group/i }));
    fireEvent.click(screen.getByRole('button', { name: /check connection/i }));
    view.unmount();
  ...
  it('times out after ninety seconds and offers retry', async () => {
    ...
    expect(
      screen.getByText('We have not received confirmation from MAX yet.')
    ).not.toBeNull();

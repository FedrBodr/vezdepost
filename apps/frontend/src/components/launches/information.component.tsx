...
const Valid: FC = () => (
  ...
  >
    <path
      d="M6 7.33333L8 9.33333L14.6667 2.66667M10.6667 2H5.2C4.0799 2 3.51984 2 3.09202 2.21799C2.71569 2.40973 2.40973 2.71569 2.21799C2 3.51984 2 4.07989 2 5.2V10.8C2 11.9201 2 12.4802 2.21799 12.908C2.40973 13.2843 2.71569 13.5903 3.09202 13.782C3.51984 14 4.07989 14 5.2 14H10.8C11.9201 14 12.4802 14 12.908 13.782C13.2843 13.5903 13.5903 13.2843 13.782C14 12.4802 14 11.9201 14 10.8V8"
      stroke="#00EB75"
      strokeWidth="1.2"
      strokeLinecap="round"
...
const counterUnit = (counter: EditorCounterV2): string => {
  switch (counter.limit.unit) {
    case 'graphemes':
      return '';
...
const counterText = (counter: EditorCounterV2): string =>
  `${counter.measured}/${counter.limit.max}${counterUnit(counter)}`;

export const InformationComponent: FC<{
  capability: EditorCapabilityV2;
  isPicture: boolean;
}> = ({ capability, isPicture }) => {
  const t = useT();
  const { isGlobal, selectedIntegrations } = useLaunchStore(
    useShallow((state) => ({
      isGlobal: state.current === 'global',
      selectedIntegrations: state.selectedIntegrations,
    }))
  );
  const primaryCounter = capability.counters[0];
  const isValid = useMemo(() => {
    if (!isPicture && !capability.sourceHasContent) {
      return false;
    }
    return !capability.blocking;
  }, [capability.blocking, capability.sourceHasContent, isPicture]);

  if (
    isGlobal &&
    selectedIntegrations.length > 0 &&
    capability.destinations.length === 0
  ) {
    return null;
  }
  ...
  return (
    ...
    >
      ...
      {primaryCounter && (
        <div
          className={clsx(
            'text-[10px] font-[600] flex justify-center items-center whitespace-nowrap',
            !isValid && 'text-white'
          )}
        >
          {counterText(primaryCounter)}
        </div>
      )}
      {((isGlobal && capability.destinations.length > 0) || !isValid) && (
        <svg
      ...
      {((isGlobal && capability.destinations.length > 0) || !isValid) && (
        ...
        >
          {!isPicture && !capability.sourceHasContent && (
            <div
              className={clsx(
                'text-sm text-[#FF3F3F] whitespace-nowrap',
                isGlobal && capability.counters.length > 0 && 'mb-[12px]'
              )}
            >
              {t(
          ...
          {isGlobal && (
            <div className="grid grid-cols-[auto_auto_auto] text-[14px] font-[500] gap-[8px] items-center">
              {capability.counters.map((counter) => {
                const exceeded = counter.measured > counter.limit.max;
                return (
                  <Fragment
                    key={`${counter.targetIntegrationId}-${counter.fieldKey}`}
                  >
                    <div>
                      <SafeImage
                        src={`/icons/platforms/${counter.destination}.png`}
                        alt={counter.destinationName}
                    ...
                    <div
                      className={clsx(
                        'whitespace-nowrap',
                        exceeded && 'text-[#FF3F3F]'
                      )}
                    >
                      {counter.destinationName} (
                      {capitalize(counter.destination.split('-')[0])}) ·{' '}
                      {counter.fieldLabel}:
                    </div>
                    <div
                      className={clsx(
